...
.directive('dateScroll',function(){
    return {
        restrict: 'E',
        template: '<input id="beginTime" class="kbtn" />',
        link: function(scope,element,attributes) {
            ...
            function doDate(options,Ycallback,Ncallback) {
                // var element = $(this);
                ...
                //用户选项覆盖插件默认选项
                var opts = clone(defaultOptions,options);
                if(opts.theme === 'datetime') {datetime=true}
                if(!opts.show) {
                    element.unbind('click');
                } else {
                    //绑定事件（默认事件为获取焦点）
                    element.bind(opts.event,function () {
                ...
                function resetInitDete(){
                    ...
                    else if(element.val()===""){return false;}
                    initY = parseInt(element.val().substr(0,4))-opts.beginyear;
                    initM = parseInt(element.val().substr(5,2));
                    initD = parseInt(element.val().substr(8,2));
                }
                function bindButton(){
                    resetIndex();
                    $("#dateconfirm").unbind('click').bind('click',function () {
                        ...
                        if(Ycallback===undefined){
                            if(docType){element.val(datestr);}else{element.html(datestr);}
                        }else{
                            Ycallback(datestr);
                        }
                        $("#datePage").css({display:'none'});
                        $("#dateshadow").css({display:'none'});
                    });
                ...
                function showdatetime(){
                    ...
                    addTimeStyle();
                    $("#datescroll_datetime").css({display:'block'});
